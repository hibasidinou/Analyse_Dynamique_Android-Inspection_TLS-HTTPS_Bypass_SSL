...
Java.perform(function(){
  ...
  }catch(e){ /* OkHttp non présent */ }

  // 5) WebView: ignorer les erreurs SSL (attention effets de bord)
  try{
    const WVC = Java.use('android.webkit.WebViewClient');
    if (WVC.onReceivedSslError) WVC.onReceivedSslError.implementation = function(view, handler, error){ ok('WebView onReceivedSslError -> proceed'); handler.proceed(); };
  }catch(e){}
